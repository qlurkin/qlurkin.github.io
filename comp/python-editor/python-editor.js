...
// THIS FILE HAS BEEN MODIFIED TO BECOME AN ES MODULE
...
// Apply python plugin to CodeMirror
python(CodeMirror);

const CodeMirror$1 = CodeMirror;
...
class PythonEditor extends HTMLElement {
	...
	constructor() {
		...
		this.activeEditor = this.activeEditor = 'Réponse';
	...
	_render() {
		return html`
			...
			<style>
				@import "/comp/python-editor/css/codemirror.css";
				@import "/comp/python-editor/css/cobalt.css";
				@import "/comp/python-editor/css/editor.css";
			</style>
			<div>
				<div id='top'>
					<header>
						<nav>
							${this.editors.map(editor => html`
								<div class="tab ${editor.title === this.activeEditor ? 'active' : ''}">
									<button data-target="${editor.title}" @click="${(event) => {this._tabFocus(event);}}">${editor.title}</button>
									${editor.removable ?
										html`<button data-target="${editor.title}" @click="${(event) => {this._deleteFile(event);}}">&times;</button>` :
										html`` }
								</div>
							`)}
						</nav>
				...
				<div id="overlay" class="${this.newFileDialogShown ? 'show' : ''}">
					<form @submit="${(event) => {this._newFile(event);}}">
	...
	_addEditor(title, content, mode, removable) {
		...
		const editor = CodeMirror$1(wrapper, {
			lineNumbers: true,
			mode: mode,
			theme: 'cobalt',
			value: content,
	...
	run() {
		...
		run(this.value, fs).then(res => {
			this.output = res.stdout;

			Object.keys(res.fs).map((title) => {
				const editor = this._getEditor(title);

				if(editor === undefined) {
					this._addEditor(title, res.fs[title], 'plain', true);
				}
				else {
					editor.editor.setValue(res.fs[title]);
				}
			});
