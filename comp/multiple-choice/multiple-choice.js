...
class multipleChoice extends HTMLElement {
    ...
    _render() {
        return html`<slot @option-set="${(event) => {this._onOptionSet(event);}}"></slot>`
    }
    ...
    _shuffle() {
        /*const options = this.querySelectorAll('an-option')
        const optionsArray = []
        ...
        const options = this.querySelectorAll('an-option');
        var shuffledList = Array.from(options).sort((a,b) => {
            return Math.floor((Math.random() * 2)) == 0 ? -1 : 1
        });
        shuffledList.forEach((option) => {
            this.appendChild(option);
        });
    }

    _onOptionSet(event) {
        this.querySelectorAll('an-option').forEach((option) => {
    ...
    set value(v) {
        this.querySelector(`an-option[value=${v}]`).checked = true;
    }
...
class anOption extends HTMLElement {
    constructor () {
        super();
        this.attachShadow({mode: 'open'});
        this._checked = false;
        ...
        return html`
            ...
            <div id='root' @click="${() => { this.toggle(); }}">
                <svg viewBox="0 0 64 64">
                    <polygon class="${this.checked ? '' : 'hidden'}" points="24.966,48.968 9.507,31.616 12.493,28.955 25.034,43.032 51.538,14.635 54.462,17.365"/>
                    <path d="M56,64H8c-3.888,0-8-4.112-8-8V8c0-3.888,4.112-8,8-8h48c3.888,0,8,4.112,8,8v48C64,59.888,59.888,64,56,64z M8,4 C6.355,4,4,6.355,4,8v48c0,1.645,2.355,4,4,4h48c1.645,0,4-2.355,4-4V8c0-1.645-2.355-4-4-4H8z"/>
                </svg>
                <slot></slot>
            </div>
        `
    ...
    set checked(value) {
        ...
        this.update();
        if(this._checked) {
            this.dispatchEvent(new Event('option-set', {bubbles: true, composed: true}));
        }
        else {
            this.dispatchEvent(new Event('option-clear', {bubbles: true, composed: true}));
        }
    }
...
customElements.define('an-option', anOption);
