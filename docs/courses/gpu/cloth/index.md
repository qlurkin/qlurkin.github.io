...
In this projet you will create a simulation of cloth in Rust. The cloth will be
just a subdivided plane.
...
The cloth geometry and the springs connectivity will be generated by Rust code
on the CPU. The simulation must run on the GPU with a Compute Shader.
Simulations parameters must come from the Rust code via an Uniform Buffer.
...
$x_(s_r) \) the position of the top after repositioning. Once the
repositioning is done, the speed must be updated. $$arrow(v)
= (arrow(x\_(s_r)) - arrow(x_s))/(Delta t)$$
