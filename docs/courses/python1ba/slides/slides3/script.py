...
def code_step(
    src, lines, vars, terminal=" ", code_size=0.9, term_size=1.0, disclaimer=None
):
    ...
    if disclaimer is not None:
        disclaimer_markup = f'<p class="disclaimer">{disclaimer}</p>'
    return f'<div class="row code-step"><div class="span6 middle" style="font-size: {code_size}em;">{left}</div><div class="span6 middle">{disclaimer_markup}{right}</div></div>'
