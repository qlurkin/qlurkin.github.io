...
export function aggregation(context) {
   const size = 5
   context.polygone([[-size, -size/2], [0, 0], [-size, +size/2], [-2*size, 0]]).fill('white').stroke({color: 'black'})
}

export function composition(context) {
   const size = 5
   context.polygone([[-size, -size/2], [0, 0], [-size, +size/2], [-2*size, 0]]).fill('black').stroke({color: 'black'})
}
