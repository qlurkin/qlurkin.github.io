  ...
  showMenu(event.offsetX, event.offsetY, [
    {
      ...
      action: () => {
        form_modal(
          document.getElementById('chip-modal'),
          {
            name: getCurrentName(),
...
let startWire = null
const ghostLine = canvas
  .line(0, 0, 0, 0)
  .stroke({ width: 3, color: 'black', opacity: 0 })
  .addClass('ghost')
...
colorPicker(
  document.querySelector('#chip-modal form #color-picker'),
  100,
  50,
  65,
  (r, g, b) => {
...
if (current !== undefined) fromJson(current)
