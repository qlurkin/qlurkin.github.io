...
import { parse, join, resolve, relative, basename } from 'path'
...
export async function pandoc(path) {
  return new Promise(async (resolve, reject) => {
    ...
    const filter_args = filters
      ...
      .join(' ')
    const child = exec(
      `pandoc -f markdown -t html -s --template ${join(root, 'pandoc', 'template.html')}  --section-divs -M document-css=false ${filter_args}  --mathml`,
...
export function make_zip(path, files) {
  ...
  for (const file of files) {
    zip.addLocalFile(file, folder, basename(file))
  }
